...
@RequiredParameters(parameters = {"b", "k" , "delta"})
public class BM25_6_ScoringIterator
    ...
    public BM25_6_ScoringIterator(NodeParameters np, LengthsIterator ls, CountIterator it)
            throws IOException {
        super(np , ls , it);

        // statistics for BM25-version-6 scoring
        b = np.get("b", 0.75);
        k = np.get("k", 1.2);
        delta = np.get("delta" , 0.06);
