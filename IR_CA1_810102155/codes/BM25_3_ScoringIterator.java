...
public class BM25_3_ScoringIterator
    ...
    public BM25_3_ScoringIterator(NodeParameters np, LengthsIterator ls, CountIterator it)
            throws IOException {
        super(np , ls , it);

        // statistics for BM25-version-3 scoring
        b = np.get("b", 0.75);
        k = np.get("k", 1.2);
    ...
    private double score(double count , double length) {
        return count * Math.log(1 + avgDocLength/length)
                * Math.log(documentCount/idf);
    }
