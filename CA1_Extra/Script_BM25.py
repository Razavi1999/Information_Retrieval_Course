...
coarse_search_range = {"k": range(1, 101, 10), "b": [i * 0.1 for i in range(0, 11)]}
fine_search_range = {"k": range(1, 11), "b": [i * 0.01 for i in range(0, 101)]}
...
def run_search(k_values, b_values):
    # Initialize a list to store the results
    results = []
    for k in k_values:
        for b in b_values:
            # Run the batch-search command
            search_command = f"{galago_path} batch-search --index={index_path} --requested={requested_count} --scorer=bm25 --k={k} --b={b} {queries_path}".split(' ')
            ...
            results.append({"k": k, "b": b, "map": map, 'ndcg': ndcg, 'p10':p10, 'recall':recall})
    return results

# Run the coarse search
res1 = run_search(coarse_search_range["k"], coarse_search_range["b"])

df1 = pd.DataFrame(res1)
df1.to_csv("hyperparameter_pre_results.csv", index=False)
...
fine_search_range["b"] = [i * 0.01 for i in range(max(0, int(best_b * 100) - 5), min(101, int(best_b * 100) + 6))]

# Run the fine search
res2 = run_search(fine_search_range["k"], fine_search_range["b"])

# Convert the results to a DataFrame and save it as a CSV file
df2 = pd.DataFrame(res2)
df2.to_csv("hyperparameter_tuning_results.csv", index=False)
