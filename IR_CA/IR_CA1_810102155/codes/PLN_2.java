...
public class PLN_2
    ...
    public PLN_2(NodeParameters np, LengthsIterator ls, CountIterator it)
            throws IOException {
        super(np , ls , it);

        // statistics for BM25-version-5 scoring
        b = np.get("b", 0.75);
        k = np.get("k", 1.2);
    ...
    private double score(double count , double length) {
        double multiplier = Math.log(1 + count);
        double numerator = count * Math.log((documentCount + 1) / collectionFrequency);
        double denominator =   (1 - b + (b * length / avgDocLength + 1));
        return multiplier * idf * numerator / denominator;
    }
