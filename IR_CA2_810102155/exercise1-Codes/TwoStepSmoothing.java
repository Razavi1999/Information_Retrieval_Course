...
@RequiredParameters(parameters = {"mu" , "landa"})
public class TwoStepSmoothing extends ScoringFunctionIterator
    ...
    public TwoStepSmoothing(NodeParameters p, LengthsIterator ls, CountIterator it)
            throws IOException {
        super(p, ls, it);
        ...
        max = twoStepScore(p.getLong("maximumCount"), p.getLong("maximumCount"));
    ...
    public double score(ScoringContext c) {
        ...
        return twoStepScore(count, length);
    }

    private double twoStepScore(double count, double length) {
        double numerator = (1 - landa) * (count + mu * background) + (landa * landa * (mu + length) * background);
